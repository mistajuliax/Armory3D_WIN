...
class Decimal(object):
    ...
    def _cmp(self, other):
        ...
        if not self:
            return -((-1)**other._sign) if other else 0
        if not other:
            return (-1)**self._sign
    ...
    def __mul__(self, other, context=None):
        ...
        if self._is_special or other._is_special:
            ...
            if self._isinfinity():
                return (
                    _SignedInfinity[resultsign]
                    if other
                    else context._raise_error(InvalidOperation, '(+-)INF * 0')
                )
            ...
            if other._isinfinity():
                return (
                    _SignedInfinity[resultsign]
                    if self
                    else context._raise_error(InvalidOperation, '0 * (+-)INF')
                )
    ...
    def __truediv__(self, other, context=None):
        ...
        if not other:
            return (
                ...
                if self
                else context._raise_error(DivisionUndefined, '0 / 0')
            )
